...
export const calificacionesService = {
  ...
  async getDetalles(calificacionId: number) {
    const response = await api.get<CalificacionDetalle[]>(`/calificaciones-detalle/${calificacionId}`);
    return response.data;
  },

  async createDetalle(data: Omit<CalificacionDetalle, 'calificacionDetalleId'>) {
    const response = await api.post<CalificacionDetalle>('/calificaciones-detalle', data);
    return response.data;
  },

  async updateDetalle(id: number, data: Partial<CalificacionDetalle>) {
    const response = await api.put<CalificacionDetalle>(`/calificaciones-detalle/${id}`, data);
    return response.data;
  },
